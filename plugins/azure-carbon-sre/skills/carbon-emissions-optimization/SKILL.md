...
7. When Azure API Management (`Microsoft.ApiManagement/service`) appears as a contributor, evaluate its [sustainability capabilities](https://learn.microsoft.com/en-us/azure/api-management/sustainability) (preview) before proposing a capacity change:
   - **Traffic shaping**: policies that read the current region's carbon-intensity context and throttle, delay, or reroute non-critical calls when intensity is high. This is available on classic tiers (Developer, Basic, Standard, Premium) and does not require a multi-region deployment, so recommend evaluating it for any APIM contributor.
   - **Traffic shifting**: the built-in [backend load balancer across `additionalLocations`](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-deploy-multi-region), weighted to prefer lower-carbon-intensity regions while preserving failover. This only applies to a Premium-tier [multi-region deployment](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-deploy-multi-region); confirm this first by checking for `additionalLocations` in the resource definition, or a [`Capacity` metric](https://learn.microsoft.com/en-us/azure/api-management/api-management-capacity) split across more than one value of the `Location` dimension.
   - Confirm the SKU and, for traffic shifting, the `additionalLocations` count as resource proof before recommending either capability. Treat both as a policy/configuration change, not a capacity or SKU change, and still require a rollback and validation step.
   - Apart from these, apply the standard capacity decision from step 6, using the [`Capacity` metric](https://learn.microsoft.com/en-us/azure/api-management/api-management-capacity): scale in, or configure [Azure Monitor autoscale](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-autoscale) (Basic, Standard, and Premium tiers only, and only for the primary location in a multi-region deployment).
...
8. `### API Management sustainability` for any Azure API Management candidate: recommend evaluating policy-based traffic shaping regardless of region count, and additionally recommend load-balanced traffic shifting when `additionalLocations` or a multi-location `Capacity` metric shows a multi-region deployment; state the SKU and region evidence for each. Also apply the standard capacity decision (scale-in or autoscale) when the `Capacity` metric is consistently low.
