...
7. For Azure API Management (`Microsoft.ApiManagement/service`) contributors, evaluate [sustainability capabilities](https://learn.microsoft.com/en-us/azure/api-management/sustainability) (preview) as recommendations in their own right, alongside the standard capacity decision from step 6:
...
8. For eligible API Management contributors on the Developer, Basic, Standard, or Premium tier, evaluate **traffic shaping**: policies that read the current region's carbon-intensity context and throttle, delay, or reroute non-critical calls when intensity is high. It does not depend on capacity, SKU, or region count.
...
10. For API Management capacity, follow the independent standard capacity decision from step 6. The [`Capacity` metric](https://learn.microsoft.com/en-us/azure/api-management/api-management-capacity) applies to every SKU except Consumption (which autoscales automatically). For Developer, Basic, Standard, and Premium tiers, recommend scale-in or [Azure Monitor autoscale](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-autoscale) when supported; autoscale is available only on Basic, Standard, and Premium and covers only the primary location in a multi-region deployment. Do not skip this track when traffic shaping or traffic shifting is recommended.
