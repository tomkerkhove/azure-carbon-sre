...
8. After confirming limited-preview enrollment and regional availability, evaluate [**traffic shaping**](https://learn.microsoft.com/en-us/azure/api-management/sustainability) for eligible API Management contributors on the Developer, Basic, Standard, or Premium tier: policies that read the current region's carbon-intensity context and throttle, delay, or reroute non-critical calls when intensity is high. It does not depend on capacity, SKU, or region count.
