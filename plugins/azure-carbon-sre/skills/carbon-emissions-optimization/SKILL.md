...
7. When Azure API Management (`Microsoft.ApiManagement/service`) appears as a contributor, treat its [sustainability capabilities](https://learn.microsoft.com/en-us/azure/api-management/sustainability) (preview) as recommendations in their own right, alongside the standard capacity decision from step 6, not as a gate that only applies when a capacity change is on the table:
   - **Traffic shaping** (always applicable): policies that read the current region's carbon-intensity context and throttle, delay, or reroute non-critical calls when intensity is high. This is available on the Developer, Basic, Standard, and Premium tiers and does not depend on capacity, SKU, or region count, so recommend evaluating it for every APIM contributor on those tiers.
   - **Traffic shifting** (multi-region only): the built-in [backend load balancer across `additionalLocations`](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-deploy-multi-region), weighted to prefer lower-carbon-intensity regions while preserving failover. Confirm a multi-region deployment first by checking for `additionalLocations` in the resource definition, or a [`Capacity` metric](https://learn.microsoft.com/en-us/azure/api-management/api-management-capacity) split across more than one value of the `Location` dimension; this capability requires the Premium tier.
   - **Capacity** (independent track): the [`Capacity` metric](https://learn.microsoft.com/en-us/azure/api-management/api-management-capacity) applies to every SKU except Consumption (Consumption is not measured by `Capacity` and autoscales automatically instead). Apply the standard capacity decision from step 6 for the Developer, Basic, Standard, and Premium tiers — scale in, or configure [Azure Monitor autoscale](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-autoscale) (Basic, Standard, and Premium tiers only; the Developer tier does not support autoscale, and only the primary location is covered in a multi-region deployment). Do not skip this when traffic shaping or traffic shifting is already recommended; they are complementary, not substitutes.
   - Confirm the SKU and, for traffic shifting, the `additionalLocations` count as resource proof before recommending it. Treat traffic shaping and traffic shifting as a policy/configuration change, not a capacity or SKU change, and still require a rollback and validation step for every recommendation.
...
8. `### API Management sustainability` for any Azure API Management candidate, presented as recommendations in their own right rather than only when a capacity change is proposed: recommend policy-based traffic shaping regardless of region count or capacity signal; additionally recommend load-balanced traffic shifting when `additionalLocations` or a multi-location `Capacity` metric shows a multi-region deployment; and still apply the standard capacity decision (scale-in or autoscale) from the `Capacity decision` section as a complementary, independent track. State the SKU and region evidence for each recommendation.
