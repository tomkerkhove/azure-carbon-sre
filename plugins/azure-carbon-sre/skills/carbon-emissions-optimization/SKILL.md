...
7. When Azure API Management (`Microsoft.ApiManagement/service`) appears as a contributor, treat its [sustainability capabilities](https://learn.microsoft.com/en-us/azure/api-management/sustainability) (preview) as recommendations in their own right, alongside the standard capacity decision from step 6, not as a gate that only applies when a capacity change is on the table:
   - **Traffic shaping** (always applicable): policies that read the current region's carbon-intensity context and throttle, delay, or reroute non-critical calls when intensity is high. This is available on classic tiers (Developer, Basic, Standard, Premium) and does not depend on capacity, SKU, or region count, so recommend evaluating it for every APIM contributor.
   ...
   - **Capacity** (independent track): apply the standard capacity decision from step 6 using the [`Capacity` metric](https://learn.microsoft.com/en-us/azure/api-management/api-management-capacity) — scale in, or configure [Azure Monitor autoscale](https://learn.microsoft.com/en-us/azure/api-management/api-management-howto-autoscale) (Basic, Standard, and Premium tiers only, and only for the primary location in a multi-region deployment). Do not skip this when traffic shaping or traffic shifting is already recommended; they are complementary, not substitutes.
