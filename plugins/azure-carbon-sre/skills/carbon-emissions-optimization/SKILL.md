...
7. When Azure API Management (`Microsoft.ApiManagement/service`) appears as a contributor, treat its [sustainability capabilities](https://learn.microsoft.com/en-us/azure/api-management/sustainability) (preview) as recommendations in their own right, alongside the standard capacity decision from step 6, not as a gate that only applies when a capacity change is on the table:
   - **Traffic shaping** (always applicable after preview eligibility is confirmed): policies that read the current region's carbon-intensity context and throttle, delay, or reroute non-critical calls when intensity is high. This is available on the Developer, Basic, Standard, and Premium tiers and does not depend on capacity, SKU, or region count, so recommend evaluating it for every eligible APIM contributor on those tiers.
   ...
   - Before recommending either sustainability capability, verify that the subscription is enrolled in the limited preview and that each target region supports the preview. If either proof is unavailable, state `No change recommended yet` and name preview eligibility verification as the next step.
   - Confirm the SKU. For traffic shifting, use either the `additionalLocations` count or the multi-location `Capacity` metric as deployment proof; state any unavailable resource proof separately.
   - Treat traffic shaping and traffic shifting as a policy/configuration change, not a capacity or SKU change, and still require a rollback and validation step for every recommendation.
