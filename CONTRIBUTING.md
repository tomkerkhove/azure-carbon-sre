...
Use semantic PR titles in this exact format:

```text
<sem>: <description> (Generated by SRE Agent)
```

Use a lowercase conventional-commit semantic prefix: `feat`, `fix`, `docs`, `refactor`, `test`, or `chore`. For example: `fix: Standardize Carbon skill presentation (Generated by SRE Agent)`.

## Copilot review

Before considering a pull request complete, request a Copilot review and address all actionable feedback. Re-run the relevant validation after each revision, request another Copilot review, and continue this cycle until Copilot signs off with no unresolved review feedback.
